...
def main(args):
  ...
  testing_dta = datasets.FashionMNIST(root='data', train=False, download=True, transform=ToTensor(),)
  ...
  class NeuralNetwork(nn.Module):
    def __init__(self, hidden_size1, hidden_size2):
      super(NeuralNetwork, self).__initi__()
  ...
  optimizer = torch.optim.SGD(model.parameter(), lr=args['lr'])
  ...
  for epoch in range(10):
    ...
    print(test_acc)
    nni.report_intermediate_result(test_acc) #TODO:add nni 

  print('final_accuracy:', test_acc)
  nni.report_final_result(test_acc) #TODO: add nni

  if __name__ == '__main__':
    #params = {'batch_size': 32,'hidden_size1': 128,'hidden_size2': 128, 'lr': 0.001,'momentum': 0.5}
    params = nni.get_next_parameter() #TODO: ADD NNI
    main(params)
